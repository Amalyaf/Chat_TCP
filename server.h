...
private:
	...
	socklen_t length;
	int socket_file_descriptior, connection, bind_status, connection_status;
...
public:
	...
	int enter_login(char* login); // авторизация пользователя
	void enter_password(char login[], char password[]);
	...
	int start(); // начало работы программы
	void server_start();
	void client_start();
	void socket_close();
};
